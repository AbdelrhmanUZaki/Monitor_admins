...
from pyrogram.types import ChatMember, ForumTopicCreated, Message
...
def add_new_user(user_id, name, user_username, user_joined_date):
    ...
    if not existing_user:
        ...
        cur.execute(
            "INSERT INTO bot_users (tele_id, name, username, date, current_step) VALUES (?, ?, ?, ?, ?)",
...
async def on_join(client, message: Message):
    ...
    """
    cur.execute(
        "SELECT current_step FROM bot_users WHERE tele_id = ?", (message.from_user.id,)
    )
...
async def add_source_group(message):
    ...
    if message.new_chat_members and bot_id in [
        member.id for member in message.new_chat_members
    ]:
...
def reset_current_step(tele_id):
    ...
    current_step = 1  #
    cur.execute(
        "UPDATE bot_users SET current_step = ? WHERE tele_id = ?",
        (current_step, tele_id),
    )
    conn.commit()
...
async def get_group_admins(
    ...
    async for admin in app.get_chat_members(
        source_group_id, filter=enums.ChatMembersFilter.ADMINISTRATORS
    ):
        admin: ChatMember
    ...
    conn.commit()
    await inform_about_new_combination(
        message, source_group_name, destination_group_name
    )
...
async def forward_admin_messages(client, message: Message):
    global admins_mapping
    if (
        message.from_user.id in admins_mapping
        and message.chat.id in admins_mapping[message.from_user.id]
    ):
...
async def inform_about_new_combination(
    message, source_group_name, destination_group_name
):
    """
...
async def get_source_group_info(user_id):
    cur.execute(
        "SELECT source_group_id, source_group_name from tmp WHERE tele_id = ?",
        (user_id,),
    )
...
async def get_source_chat_id(source_message: Message):
    ...
    cur.execute(
        ...
        (bot_user_id, source_group_id, source_group_name),
    )
    conn.commit()
