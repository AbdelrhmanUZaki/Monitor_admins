...
def fetch_all_db_admin_ids():
    ...
    #}
    admins_mapping = {}
    cur.execute("SELECT admin_id, source_group_id, destination_group_id, destination_thread_id from combination")
    admins_fetched = cur.fetchall()
    for admin_id, source_group_id, destination_group_id, destination_thread_id in admins_fetched:
        if admin_id not in admins_mapping:
            admins_mapping[admin_id] = {}
        admins_mapping[admin_id][source_group_id] = (destination_group_id, destination_thread_id)
